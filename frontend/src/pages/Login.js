...
export default function Login() {
  ...
  useEffect(() => {
    ...
    googleCallbackRef.current = handleCredential;
          ...
          <div className="relative rounded-[28px] sm:rounded-[32px] overflow-hidden"
            ...
            <div className="px-5 sm:px-7 py-5 sm:py-6 relative">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                {/* Google Sign In — single click via prompt() */}
                <button type="button"
                  onClick={() => window.google?.accounts.id.prompt()}
                  disabled={!googleReady}
                  className="w-full flex items-center justify-center gap-3 rounded-2xl transition-all duration-200"
                  style={{
                    height: 46,
