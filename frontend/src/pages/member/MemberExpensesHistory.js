    ...
    <div className="space-y-3 pb-8">
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {[{label:'Grocery',val:gTotal,c:'#34d399'},{label:'Other',val:oTotal,c:'#fbbf24'},{label:'Total',val:gTotal+oTotal,c:'#60a5fa'}].map(({label,val,c})=>(
          <div key={label} className="rounded-xl px-3 py-2" style={glass}>
            <p className="text-[9px] text-slate-500 mb-0.5">{label}</p>
            <p className="text-xs font-bold truncate" style={{ color: c }}>₹{val.toFixed(0)}</p>
          </div>
        ))}
      </div>
        ...
        <div className="space-y-1.5">
          {groups.length===0
            ...
            : groups.map(group=>{
                ...
                const ac=isLunch?'#fbbf24':'#a78bfa', ab=isLunch?'rgba(251,191,36,0.08)':'rgba(139,92,246,0.08)', abr=isLunch?'rgba(251,191,36,0.18)':'rgba(139,92,246,0.18)';
                return (
