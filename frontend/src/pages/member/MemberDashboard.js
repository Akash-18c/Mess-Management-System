...
export default function MemberDashboard() {
  ...
  const { lunch, dinner, mealRate, estimatedBill, myOtherCharges, masiPerMember, advance, totalCollected, memberMealCounts, summary, individualCosts, gasCharge, riceCharge } = data;
        ...
        <div className="rounded-2xl p-5" style={glass}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              { icon: IndianRupee, value: `₹${(estimatedBill + masiSalary + myOther + myGasCharge + myRiceCharge).toFixed(2)}`, label: 'My Bill',
                sub: [
                  masiSalary > 0   ? `Masi ₹${masiSalary.toFixed(2)}`   : '',
                  myGasCharge > 0  ? `Gas ₹${myGasCharge.toFixed(2)}`   : '',
                  myRiceCharge > 0 ? `Rice ₹${myRiceCharge.toFixed(2)}` : '',
                  myOther > 0      ? `Other ₹${myOther.toFixed(2)}`     : '',
