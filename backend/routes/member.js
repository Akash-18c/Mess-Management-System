...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const [meals, payments, summary, bill, allMeals, allPayments, groceries, others, members, bills] = await Promise.all([
      Meal.find({ memberId: req.user._id, month, year }),
      Payment.find({ memberId: req.user._id, month, year }),
      MonthlySummary.findOne({ month, year }),
      Bill.findOne({ memberId: req.user._id, month, year }),
    ...
    // Individual cost table — ALL active users, computed from meals directly (not just bills)
    ...
    res.json({ lunch, dinner, guestMeals, mealRate, estimatedBill, advance, bill, todayExpense, totalCollected, memberMealCounts, summary, individualCosts });
...
router.get('/month-data/:month/:year', async (req, res) => {
  try {
    ...
    const Meal = require('../models/Meal');
    const rn = (name) => { const m = name?.match(/^\w+\s*\((.+)\)$/); return m ? m[1] : (name || ''); };
    const [summary, allBills, allPayments, allMembers, allMeals] = await Promise.all([
      MonthlySummary.findOne({ month, year }),
      Bill.find({ month, year }).populate('memberId', 'name room'),
      Payment.find({ month, year }),
      User.find({ isActive: true }, 'name _id role'),
...
router.get('/all-history', async (req, res) => {
  try {
    ...
    // Gather all months that have ANY meal data or summary
    const [allMeals, allSummaries, allBills, allPayments, allMembers, allMasiSalaries] = await Promise.all([
    ...
    // Build unique month/year set from meals + summaries
    ...
    const result = [...monthSet].map(key => {
      ...
      const members = allMembers.map(m => {
        ...
        const totalBill = mBill ? mBill.totalBill : parseFloat(((mealCount + guestCount) * mealRate).toFixed(2));
        ...
        const dueAmount = parseFloat((totalBill + masiPerMember - advance).toFixed(2));
        if (totalMeals === 0 && advance === 0 && masiPerMember === 0) return null;
        return {
          _id: mBill?._id || m._id,
          ...
          lunchCount, dinnerCount, guestMeals, guestCharge,
          mealRate, totalBill, masiSalary: masiPerMember, advance, dueAmount,
        };
      }).filter(Boolean);
...
// All payments for a month (for history breakdown)
