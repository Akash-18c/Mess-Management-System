...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    // My other charges
    const myOtherCharges = allOtherCharges
      .filter(c => c.memberId?.toString() === req.user._id.toString())
    ...
    const todayOther   = others.filter(o => o.date?.toISOString().slice(0,10) === today).reduce((s, o) => s + o.amount, 0);
    ...
    const individualCosts = members.map(m => {
      ...
      // mealCost = meals * rate only (not total bill)
      const mealCost = parseFloat(((mealCount + guestCount) * mealRate).toFixed(2));
      // otherCharges for this member
      const memberOther = parseFloat(allOtherCharges.filter(c => c.memberId?.toString() === m._id.toString()).reduce((s, c) => s + c.amount, 0).toFixed(2));
      // totalBill from generated bill (most accurate) or calculated
      const totalMealCost = mBill ? parseFloat(mBill.totalBill.toFixed(2)) : parseFloat((mealCost + masiPerMember + memberOther).toFixed(2));
      ...
      return { _id: m._id, name: rn(m.name), role: m.role, totalMeals, perMealCost: mealRate, mealCost, otherCharges: memberOther, masiSalary: masiPerMember, totalMealCost, moneyGiven, due };
    }).filter(m => m.totalMeals > 0 || m.moneyGiven > 0 || m.masiSalary > 0 || m.otherCharges > 0);

    res.json({ lunch, dinner, guestMeals, mealRate, estimatedBill, myOtherCharges, masiPerMember, advance, todayExpense, totalCollected, memberMealCounts, summary, individualCosts });
...
router.get('/all-history', async (req, res) => {
  try {
    ...
    const result = [...monthSet].map(key => {
      ...
      const monthCharges  = allOtherCharges.filter(c => c.month == month && c.year == year);
      ...
      const members = allMembers.map(m => {
        ...
        const memberOther = parseFloat(monthCharges.filter(c => c.memberId?.toString() === m._id.toString()).reduce((s, c) => s + c.amount, 0).toFixed(2));
        const mealCost = parseFloat(((mealCount + guestCount) * mealRate).toFixed(2));
        // Use generated bill totalBill if exists, else calculate
        const totalBill = mBill ? mBill.totalBill : parseFloat((mealCost + masiPerMember + memberOther).toFixed(2));
        ...
        const otherCharges = mBill ? (mBill.otherCharges || memberOther) : memberOther;
        ...
        if (totalMeals === 0 && advance === 0 && masiPerMember === 0 && memberOther === 0) return null;
        return {
          _id: mBill?._id || m._id,
          ...
          mealCost, otherCharges, masiSalary: masiPerMember,
